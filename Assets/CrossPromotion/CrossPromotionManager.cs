...
public class CrossPromotionManager : MonoBehaviour
{
    ...
    IEnumerator GetText()
    {
        ...
        UnityWebRequest request = UnityWebRequest.Get("https://raw.githubusercontent.com/alinawaz1997/crosspromotion/refs/heads/main/CGS_AMAZON_Json/skincare.json");
        yield return request.SendWebRequest();
        if (request.isDone && request.result == UnityWebRequest.Result.Success)
        {
            if (request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
                ...
                if (GamesInfo.Instance.adsData[0].canShowCP)
                {
                    for (int i = 0; i < GamesInfo.Instance.gamesData.Count; i++)
                    {
                        if (i < maxLoads)
                        {
                            ...
                            serverPath = "https://raw.githubusercontent.com/alinawaz1997/crosspromotion/main/CGS_AMAZON_Icons/" + iconName + ".png";
